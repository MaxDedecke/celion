...
import { CheckCircle2, Circle, Copy, Loader2, Save } from "lucide-react";
    ...
    <div className="flex flex-1 min-h-0 bg-background rounded-3xl overflow-hidden border">
      ...
      <div className="flex-1 bg-background flex flex-col">
        <div className="p-4 border-b flex items-center justify-between shrink-0">
            <div>
                <h2 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Erkenntnisse & Ergebnisse</h2>
            </div>
            <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={handleCopyJson} disabled={!currentResult} className="h-8 text-xs">
                    <Copy className="h-3.5 w-3.5 mr-1.5" />
                    JSON Kopieren
                </Button>
                <Button size="sm" onClick={handleSaveResult} disabled={!currentResult || isSaving} className="h-8 text-xs min-w-[100px]">
